...
const Raptor = new Dinosaur({species:"velociraptor", diet:"carnivorous", weight:"15kg", height:"1.8m", epoch:"Late Cretaceous"})
...
console.log(Stegg.height);

// What time period did tyrannosaurus live in?
console.log(TRex.epoch);
...
graduates.forEach((item) => {
  universities.sort().push( `${item.university}`)
});
...
const unisWithUni = [];
unisWithUni.push(graduates.filter((item) => item.university.includes("Uni")));
...
populationTotal = zooAnimals.reduce((acc, currentVal) => acc + currentVal.population, 0);
